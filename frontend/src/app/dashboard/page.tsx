  ...
  const fetchScore = useCallback(
    async (rut: string) => {
      ...
      try {
        ...
        const score = scoreData.status === "fulfilled" ? scoreData.value : null;
        const person =
        ...
        if (status === 401) {
          addToast(
            "error",
            "Sesión expirada",
            "Tu token ha vencido. Por favor inicia sesión nuevamente.",
          );
        } else if (status === 403) {
          addToast(
            "error",
            "Acceso denegado",
            "Solo puedes consultar tu propio RUT.",
          );
        } else if (status === 400) {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    if (!searchRutValid) {
      addToast(
        "warning",
        "RUT inválido",
        "Verifica el dígito verificador antes de consultar.",
      );
      return;
    }
    ...
    <>
      ...
      <main className={styles.main}>
        <div className={styles.container}>
            ...
            <div className={`glass-card ${styles.searchCard} animate-slide-in`}>
              ...
              <form onSubmit={handleSearch} className={styles.searchForm}>
                ...
                <button
                  ...
                  disabled={
                    view === "loading" || !searchRut || searchRutValid === false
                  }
                >
                  {view === "loading" ? (
                    <>
            ...
            <div className={`glass-card ${styles.stateCard}`}>
              <div className={styles.loadingState}>
                ...
                <p style={{ color: "var(--text-secondary)" }}>
                  Calculando score financiero...
                </p>
                <p style={{ color: "var(--text-muted)", fontSize: 12 }}>
                  Consultando base de datos de riesgo
                </p>
              </div>
            </div>
          )}
          ...
          {view === "error" && (
            <div
              className={`glass-card ${styles.stateCard} ${styles.errorCard}`}
            >
              <svg
                width="48"
                height="48"
          ...
          {view === "score" && result && category && (
            <div className={styles.scoreLayout + " animate-fade-in"}>
              {/* Score Gauge Card */}
              <div
                className={`glass-card-elevated ${styles.gaugeCard} animate-pulse-glow`}
              >
              ...
              <div className={styles.infoColumn}>
                ...
                <div className={`glass-card ${styles.infoCard}`}>
                  ...
                  <div>
                    ...
                    {result.person?.sex &&
                      result.person.sex !== "Sin Datos" && (
                        <div className="info-row">
                          <span className="info-key">Sexo</span>
                          <span className="info-value">
                            {result.person.sex === "VAR"
                              ? "Masculino"
                              : result.person.sex === "MUJ"
                                ? "Femenino"
                                : result.person.sex}
                          </span>
                        </div>
                    ...
                    {result.person?.address &&
                      result.person.address !== "Sin Datos" && (
                        <div className="info-row">
          ...
          {view === "idle" && user.role === "admin" && (
            <div
              className={`glass-card ${styles.stateCard} ${styles.idleCard}`}
            >
              <svg
                width="56"
                height="56"
              ...
              <div>
                ...
                <p style={{ color: "var(--text-muted)", fontSize: 14 }}>
                  Ingresa un RUT chileno válido en el buscador de arriba para
                  ver el score financiero del usuario.
                </p>
              </div>
            </div>
          )}
