...
| `src/data/mockUsers.ts` | Base de datos de usuarios de prueba con roles diferenciados (`user` y `admin`) |
| `src/controllers/rut.controller.ts` | Endpoint público `/person/:rut` para consulta de datos asociados al RUT |
| `tests/*.test.ts` | Suite completa de tests unitarios e integración con Jest y Supertest |
